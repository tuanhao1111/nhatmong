 ...
 * utils.js
 */
function showToast(msg,type,dur){dur=dur||3000;var e=document.querySelector('.toast');if(e)e.remove();var t=document.createElement('div');t.className='toast';t.textContent=msg;t.style.cssText='position:fixed;bottom:24px;right:24px;z-index:9999;padding:12px 20px;border-radius:8px;font-size:14px;font-weight:500;max-width:360px;box-shadow:0 4px 20px rgba(0,0,0,0.5);transition:opacity 0.3s;background:'+(type==='error'?'#2d1515':type==='success'?'#0f2d1f':'#1a1a2e')+';color:'+(type==='error'?'#f87171':type==='success'?'#4ade80':'#e8e8f0')+';border:1px solid '+(type==='error'?'#7f1d1d44':type==='success'?'#14532d44':'#2a2a4044');document.body.appendChild(t);setTimeout(function(){t.style.opacity='0';setTimeout(function(){if(t.parentNode)t.remove();},300);},dur);}
